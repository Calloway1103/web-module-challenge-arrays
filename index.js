...
function addFlavor(arrayFlavor, stringFlavor){
arrayFlavor.unshift(stringFlavor)
return arrayFlavor;
}
...
function removeLastFlavor(arrayFlavor){
 arrayFlavor.pop();
 return arrayFlavor
}
...
function getFlavorByIndex(arrayFlavor, number){
  return arrayFlavor[number];
}
...
function filterByWord(arrayFlavor, flavor){
  let filteredArray = [];
  for(let i = 0; i < arrayFlavor.length; i++){
    if(arrayFlavor[i].includes(flavor)){
      filteredArray.push(arrayFlavor[i]);
    }
  }
